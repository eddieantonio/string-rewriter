...
module.exports = class Persist {
  ...
  addOccurrences(hash, occurrences) {
    ...
    return new Promise((resolve, reject) => {
      ...
      client.incr(FILES_SEEN);
      client.hset(STRING_TOTAL, hash, stringsTotal);
      client.hset(STRING_MATCHING, hash, matchingStrings);

      for (const [name, score] of scoreGrammars(occurrences)) {
        client.zincrby(SAW_GRAMMAR, score, name);
        client.sadd(`grammar:${name}`, hash);
      }

      resolve(occurrences.length);
    });
  }
